...
const text=`<b>◈ WALZY OWNER • DARK PRO ◈</b>
<i>Exclusive Control Panel</i>

${bq(`👑 <b>IDENTITY</b>
├ Owner: <b>${esc(user.first_name)}</b>
├ ID: <code>${chatId}</code>
└ Access: <code>GRANTED • SECURE</code>

📊 <b>LIVE STATS</b>
├ Users Valid: <code>${validUsers.length}</code>
├ VIP Active: <code>${premiumCount}</code>
├ Today Order: <code>${todayOrders}</code>
...
└ Income: <code>${fmtMoney(revenue)}</code>

⏰ <b>${now.toLocaleTimeString('id-ID',{timeZone:'Asia/Jakarta'})} WIB</b>
⚡️ System Ultra Fast • Zero Lag`)}

<i>Semua fitur owner via WebApp • 🚀 walzy</i>`;
return {
text,
opts:{
parse_mode:'HTML',
reply_markup:{
inline_keyboard:[
[{text:'🎨 Owner WebApp • Full Control',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],
[{text:'📦 Pending ('+pendingOrders+')',callback_data:'menu_order'},{text:'👥 Users ('+validUsers.length+')',callback_data:'owner_users'}],
[{text:'🎟️ Voucher',callback_data:'owner_voucher'},{text:'📊 Stats',callback_data:'menu_stats'}],
...
const status=isPremium(user) ? `💎 VIP • ${getPremiumLeft(user)} hari lagi` : `🎫 Gratis • ${can.remaining}/3 hari ini`;
const text=`<b>◈ WALZY STORE ◈</b>
<i>Premium Fix Merah Bot</i>

${bq(`✨ <b>Halo, ${esc(user.first_name)}!</b>
...
├ Today: <code>${user.dailyFix.count||0}/3</code>
└ Join: <code>${new Date(user.joinedAt).toLocaleDateString('id-ID')}</code>

🎯 <b>HOW TO USE</b>
├ Buka WebApp di bawah
├ Pilih paket premium
├ Bayar DANA ${config.DANA_NUMBER}
└ Upload bukti • ACC 5 menit`)}

<i>Tap WebApp untuk pengalaman premium • 🚀 walzy</i>`;
return {
text,
opts:{
parse_mode:'HTML',
reply_markup:{
inline_keyboard:[
[{text:'🎨 Buka Walzy Store • Premium',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],
...
if(!checkRateLimit(chatId)){ return bot.sendMessage(chatId, `⏳ <b>Sabar</b>\n${bq('Jangan spam\nTunggu 1 detik')}`, {parse_mode:'HTML'}); }
...
if(text.length>1000) return bot.sendMessage(chatId, `❌ <b>Panjang</b>\n${bq('Max 1000')}`, {parse_mode:'HTML'});
const unique=getUniqueUsers(db.users);
let sent=0, failed=0;
const statusMsg=await bot.sendMessage(chatId, `📢 <b>Mulai Broadcast</b>\n${bq(`Ke ${unique.length} user`)}`, {parse_mode:'HTML'});
for(let i=0;i<unique.length;i++){
const u=unique[i];
try{ await bot.sendMessage(u.id, `📢 <b>Walzy Broadcast</b>\n${bq(esc(text))}`, {parse_mode:'HTML'}); sent++; }catch(e){ failed++; }
if(i%10===0){ try{ await bot.editMessageText(`📢 <b>Proses</b>\n${bq(`${i}/${unique.length} ✅${sent} ❌${failed}`)}`, {chat_id:chatId,message_id:statusMsg.message_id,parse_mode:'HTML'}); }catch(e){} await new Promise(r=>setTimeout(r,300)); }else{ await new Promise(r=>setTimeout(r,70)); }
}
user.awaitingBroadcast=false;
await saveDB(db);
return bot.editMessageText(`✅ <b>Selesai Broadcast</b>\n${bq(`Total ${unique.length}\n✅ ${sent} ❌ ${failed}`)}`, {chat_id:chatId,message_id:statusMsg.message_id,parse_mode:'HTML'});
}
...
await saveDB(db);
await bot.editMessageText(`❌ <b>Fix Gagal</b>\n${bq(`Nomor ${esc(text)}\n\n${esc(result.message||'Tidak respon')}`)}`, {chat_id:chatId,message_id:procMsg.message_id,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'🔁 Coba Lagi',callback_data:'menu_fix'}]]}});
}
}catch(e){
...
await saveDB(db);
return bot.sendMessage(chatId, `✅ <b>Berhasil Redeem</b>\n${bq(`🎫 ${code.toUpperCase()} ${days}H\nSampai ${new Date(user.premiumUntil).toLocaleDateString('id-ID')}`)}`, {parse_mode:'HTML'});
}
if(isValidNumber(text)){
user.awaitingNumber=true;
await saveDB(db);
const can=canUseFix(db,user);
if(!can.allowed){ user.awaitingNumber=false; await saveDB(db); return bot.sendMessage(chatId, `⏳ <b>Limit Habis</b>\n${bq('Sudah 3x\nBesok reset\nAtau VIP')}`, {parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'💎 VIP WebApp',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}]]}}); }
return bot.sendMessage(chatId, `🔧 <b>Konfirmasi Fix</b>\n${bq(`Nomor <code>${esc(text)}</code>\nSisa ${can.remaining}/3\n\nKetik YA untuk proses\nBatal untuk batal`)}`, {parse_mode:'HTML'});
}
const m=isOwner(chatId) ? getOwnerMenu(chatId,db,user) : getUserMenu(chatId,db,user);
return bot.sendMessage(chatId,m.text,m.opts);
}
...
const unique=getUniqueUsers(db.users);
let t=`👥 <b>Users Valid • ${unique.length}</b>\n\n`;
...
return bot.editMessageText(t,{chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'🎨 Owner WebApp',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
if(data==='owner_voucher' && isOwner(chatId)){
const codes=Object.values(db.codes||{}).slice(-10);
let t=`🎟️ <b>Voucher Manager</b>\n${bq(`/gen KODE HARI [KUOTA]`)} \n\n`;
if(codes.length>0){
...
return bot.editMessageText(t,{chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'🎨 Kelola di WebApp',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
if(data==='menu_stats'){
const validUsers=getUniqueUsers(db.users);
const now=new Date();
...
const revenue=db.stats.revenue||0;
const txt=`📊 <b>Statistik Walzy</b>\n${bq(`👥 Users Valid: <code>${validUsers.length}</code>\nVIP: <code>${validUsers.filter(u=>isPremium(u)).length}</code>\n\n📦 Total Order: <code>${db.stats.totalFix||0}</code>\nToday: <code>${todayOrders}</code>\nPending: <code>${pending}</code>\nSuccess: <code>${db.stats.totalSuccess||0}</code>\n\n💰 Revenue: <code>${fmtMoney(revenue)}</code>\nRate: <code>${db.stats.totalFix ? Math.round((db.stats.totalSuccess/db.stats.totalFix)*100) : 0}%</code>`)}`;
return bot.editMessageText(txt,{chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'🎨 Buka WebApp',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
...
return bot.editMessageText(`💎 <b>Paket Premium</b>\n${bq(`🔥 Starter 7H Rp 15K\n├ Pemula\n├ Unlimited fix\n└ Support\n\n⭐ Pro 30H Rp 45K Best\n├ Terlaris hemat 40%\n├ Bonus spin\n└ Unlimited\n\n👑 Sultan 90H Rp 99K\n├ Power user\n├ Prioritas\n└ Best value`)}`, {chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'🎨 Beli via WebApp',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
if(data==='menu_profile'){
const rnk=getRank(user.referralCount||0);
const can=canUseFix(db,user);
return bot.editMessageText(`👤 <b>Profil Kamu</b>\n${bq(`Name: <b>${esc(user.first_name)}</b>\nID: <code>${user.id}</code>\nUsername: @${esc(user.username||'-')}\n\n💎 Status: ${isPremium(user)?`VIP ${getPremiumLeft(user)}H`:`Free ${can.remaining}/3`}\n🏆 Rank: ${rnk.name} ${rnk.icon}\n👥 Ref: ${user.referralCount||0}\n📦 Order: ${user.totalFix||0}\n📅 ${new Date(user.joinedAt).toLocaleDateString('id-ID')}`)}`, {chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
if(data==='menu_spin'){
const canSpin=!user.lastSpin || user.lastSpin!==getTodayString();
return bot.editMessageText(`🎰 <b>Daily Spin</b>\n${bq(`${canSpin?'🎉 Bisa spin hari ini':'⏳ Sudah spin'}\n\nHadiah VIP & Bonus`)}`, {chat_id:chatId,message_id:msgId,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:canSpin?'🎰 Spin di WebApp':'✅ Sudah',web_app:{url:`${process.env.PUBLIC_URL || ''}/webapp`}}],[{text:'◁ Kembali',callback_data:'menu_main'}]]}});
}
