...
--shadow2:0 20px 50px rgba(15,23,42,0.10);
}
*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;font-family:'Plus Jakarta Sans',sans-serif}
body{background:var(--bg);color:var(--text);min-height:100vh;padding-bottom:110px;overflow-x:hidden}
.icon{width:20px;height:20px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
.loader{position:fixed;inset:0;z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:#ffffff;transition:opacity .4s ease, visibility .4s ease}
.loader.hidden{opacity:0;visibility:hidden;pointer-events:none}
.loader-logo{width:64px;height:64px;border-radius:18px;background:#0f172a;display:grid;place-items:center;color:#fff;font-family:'Outfit',sans-serif;font-weight:800;font-size:26px;letter-spacing:-.5px}
.loader-bar{width:120px;height:3px;border-radius:999px;background:rgba(15,23,42,0.06);overflow:hidden}
.loader-bar i{display:block;height:100%;width:36%;background:#0f172a;border-radius:999px;animation:bar 1s ease-in-out infinite}
@keyframes bar{0%{transform:translateX(-100%)}100%{transform:translateX(320%)}}
@keyframes in{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.9)}}
.header{position:sticky;top:0;z-index:30;height:60px;display:flex;align-items:center;justify-content:space-between;padding:0 18px;background:rgba(255,255,255,0.88);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);border-bottom:1px solid var(--bd)}
.brand{display:flex;align-items:center;gap:10px}
.brand-logo{width:32px;height:32px;border-radius:9px;background:#0f172a;display:grid;place-items:center;color:#fff;font-family:'Outfit',sans-serif;font-weight:800;font-size:16px}
.brand-name{font-family:'Outfit',sans-serif;font-weight:800;font-size:15px;letter-spacing:-.2px}
.live{display:flex;align-items:center;gap:6px;padding:5px 10px;border-radius:999px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.14);font-size:10px;font-weight:800;color:var(--emerald)}
.live b{width:6px;height:6px;border-radius:50%;background:var(--emerald);animation:pulse 1.6s infinite;display:inline-block}
.container{max-width:480px;margin:0 auto;padding:12px 12px}
.card{background:var(--card);border:1px solid var(--bd);border-radius:20px;padding:16px;box-shadow:var(--shadow)}
.badge{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;font-size:10px;font-weight:800;letter-spacing:.3px;text-transform:uppercase;border:1px solid var(--bd);background:rgba(15,23,42,0.03);color:var(--muted)}
.badge.cyan{background:rgba(6,182,212,0.08);border-color:rgba(6,182,212,0.14);color:var(--cyan)}
.badge.purple{background:rgba(139,92,246,0.08);border-color:rgba(139,92,246,0.14);color:var(--purple)}
.badge.emerald{background:rgba(16,185,129,0.08);border-color:rgba(16,185,129,0.14);color:var(--emerald)}
.badge.amber{background:rgba(245,158,11,0.09);border-color:rgba(245,158,11,0.16);color:var(--amber)}
.badge.dark{background:#0f172a;border-color:#0f172a;color:#fff}
.grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin:10px 0}
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px}
.stat{background:var(--bg);border:1px solid var(--bd);border-radius:16px;padding:12px 10px}
.stat-v{font-family:'Outfit',sans-serif;font-weight:800;font-size:17px;color:#0f172a}
.stat-l{font-size:9px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:var(--muted);margin-top:2px}
.btn{width:100%;padding:13px 14px;border-radius:14px;border:none;font-family:'Outfit',sans-serif;font-weight:800;font-size:12.5px;letter-spacing:.2px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:transform .14s ease, opacity .14s ease}
...
.btn-ghost{background:var(--bg);border:1px solid var(--bd);color:#0f172a}
.btn-amber{background:#f59e0b;color:#fff}
.input{width:100%;padding:12px 12px;border-radius:14px;background:#fbfbfd;border:1px solid var(--bd);color:#0f172a;outline:none;font-size:13px;font-weight:600}
.input:focus{border-color:var(--bd2)}
.view{display:none;opacity:0}
.view.active{display:block;opacity:1;animation:in .28s ease}
.nav{position:fixed;bottom:12px;left:50%;transform:translateX(-50%);width:calc(100% - 20px);max-width:480px;background:rgba(255,255,255,0.96);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--bd);border-radius:20px;display:flex;justify-content:space-around;padding:6px;z-index:60;box-shadow:var(--shadow2)}
.tab{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;padding:8px 4px;border-radius:14px;color:var(--muted);font-size:10px;font-weight:800;letter-spacing:.3px;cursor:pointer;transition:all .18s ease;border:1px solid transparent}
.tab.active{color:#0f172a;background:var(--bg2);border-color:var(--bd)}
.wheel-wrap{position:relative;width:260px;height:260px;margin:0 auto}
.wheel{width:260px;height:260px;border-radius:50%;border:4px solid #fff;box-shadow:0 8px 22px rgba(15,23,42,0.06), 0 0 0 1px rgba(15,23,42,0.06) inset;transition:transform 4s cubic-bezier(.15,.85,.2,1);overflow:hidden}
...
.shop-item{background:var(--card);border:1px solid var(--bd);border-radius:18px;padding:14px;box-shadow:var(--shadow);position:relative}
.shop-item.best{border-color:#0f172a}
...
.point-price{font-family:'Outfit',sans-serif;font-weight:800;font-size:12px;color:#0f172a;margin-top:6px}
.toast{position:fixed;top:12px;left:50%;transform:translateX(-50%) translateY(-10px);background:#0f172a;border:1px solid #0f172a;border-radius:16px;padding:12px 14px;display:flex;gap:10px;align-items:center;min-width:280px;max-width:92vw;box-shadow:var(--shadow2);opacity:0;pointer-events:none;transition:all .26s ease;z-index:10000;color:#fff}
...
.streak-day{width:32px;height:36px;border-radius:9px;background:var(--bg);border:1px solid var(--bd);display:grid;place-items:center;font-size:10px;font-weight:800;color:var(--muted)}
.streak-day.active{background:#0f172a;border-color:#0f172a;color:#fff}
.invoice{background:var(--bg);border:1px solid var(--bd);border-radius:16px;padding:12px;margin-bottom:8px}
.avatar{width:52px;height:52px;border-radius:14px;background:#0f172a;display:grid;place-items:center;color:#fff}
...
<div class="toast" id="toast">
  ...
  <div style="flex:1"><div id="toastTitle" style="font-family:'Outfit',sans-serif;font-weight:800;font-size:12px;color:#fff">System</div><div id="toastMsg" style="font-size:11px;color:rgba(255,255,255,0.72);font-weight:600;margin-top:1px">Message</div></div>
  <div class="toast-bar" id="toastProgress"></div>
</div>

<div class="modal" id="imageZoomModal" onclick="closeZoomModal()"><div onclick="event.stopPropagation()" style="display:flex;flex-direction:column;gap:12px;align-items:center"><img id="zoomedImageSrc" src="" style="max-width:92vw;max-height:76vh;object-fit:contain;border-radius:16px;box-shadow:var(--shadow2)"><button class="btn btn-ghost" style="width:auto;padding:9px 22px;background:#fff" onclick="closeZoomModal()">Tutup</button></div></div>
...
<div class="container">
  <div id="viewUserArea">
    <div id="viewBeranda" class="view active">
      <div class="card">
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div style="flex:1">
            ...
            <div style="display:flex;gap:5px;margin-top:7px;flex-wrap:wrap"><span class="badge dark" id="uRankBadge">BASIC</span><span class="badge" id="uStatusBadge">Gratis</span><span class="badge" id="uWeekBadge">M1</span></div>
          </div>
        </div>
        <div class="grid3">
          <div class="stat"><div class="stat-v" id="sQuota">5/5</div><div class="stat-l">Kuota</div></div>
      ...
      <div class="card">
        ...
        <button class="btn btn-dark" style="margin-top:8px" onclick="claimVoucher(this)">Tukarkan</button>
    ...
    <div id="viewProduk" class="view">
      <div style="font-family:'Outfit',sans-serif;font-weight:800;font-size:20px;margin-bottom:2px">Produk</div>
      <div style="font-size:12px;color:var(--muted);margin-bottom:12px;font-weight:600">Pilih paket & upload bukti</div>
      <div id="activeInvoiceBox"></div>
      <div class="shop-grid">
        <div class="shop-item"><div style="font-weight:800;font-size:13px">Trial 3H</div><div style="font-size:10px;color:var(--muted)">3 Hari</div><div style="font-weight:800;font-size:16px;margin-top:6px">Rp 7.000</div><button class="btn btn-dark btn-buy-pkg" style="padding:9px;font-size:11px;margin-top:8px" onclick="createOrder(3,7000,this)">Beli</button></div>
        <div class="shop-item"><div style="font-weight:800;font-size:13px">Hemat 5H</div><div style="font-size:10px;color:var(--muted)">5 Hari</div><div style="font-weight:800;font-size:16px;margin-top:6px">Rp 10.000</div><button class="btn btn-dark btn-buy-pkg" style="padding:9px;font-size:11px;margin-top:8px" onclick="createOrder(5,10000,this)">Beli</button></div>
        <div class="shop-item"><div style="font-weight:800;font-size:13px">Starter 7H</div><div style="font-size:10px;color:var(--muted)">Popular</div><div style="font-weight:800;font-size:16px;margin-top:6px">Rp 15.000</div><button class="btn btn-dark btn-buy-pkg" style="padding:9px;font-size:11px;margin-top:8px" onclick="createOrder(7,15000,this)">Beli</button></div>
        <div class="shop-item"><div style="font-weight:800;font-size:13px">Pro 14H</div><div style="font-size:10px;color:var(--muted)">Best Value</div><div style="font-weight:800;font-size:16px;margin-top:6px">Rp 25.000</div><button class="btn btn-dark btn-buy-pkg" style="padding:9px;font-size:11px;margin-top:8px" onclick="createOrder(14,25000,this)">Beli</button></div>
        <div class="shop-item best" style="grid-column:span 2;position:relative"><span class="shop-badge">BEST</span><div style="font-weight:800;font-size:15px">Sultan 30H</div><div style="font-size:11px;color:var(--muted)">Unlimited + 500 PTS</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px"><div style="font-weight:800;font-size:19px">Rp 40.000</div><button class="btn btn-dark btn-buy-pkg" style="width:auto;padding:9px 16px" onclick="createOrder(30,40000,this)">Beli Sultan</button></div></div>
      </div>
      <div class="card" style="margin-top:10px">
        ...
        <div style="font-size:11px;color:var(--muted);line-height:1.6;font-weight:600">
          ...
          4. Upload bukti<br>
          5. Tunggu verifikasi realtime<br>
          6. VIP aktif otomatis
        </div>
        <div style="margin-top:10px;padding:10px;border-radius:12px;background:var(--bg);border:1px solid var(--bd);font-size:11px;font-weight:700">DANA: <span id="danaNumber">Memuat</span> a.n <span id="danaName">WALZY</span></div>
      </div>
    </div>

    <div id="viewDaily" class="view">
      <div class="card">
        <div class="section-title">Check-in Mingguan Berbeda</div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <span class="badge" id="dailyStatusBadge">Belum</span>
        </div>
        <div style="font-size:10px;color:var(--muted);margin-bottom:8px;font-weight:600" id="weeklyInfo">Memuat poin mingguan</div>
        ...
        <button class="btn btn-dark" id="checkinBtn" style="margin-top:10px" onclick="triggerCheckin(this)">Check-in</button>
      </div>
      ...
      <div class="card">
        ...
        <button class="btn btn-dark" id="spinBtn" style="margin-top:12px" onclick="triggerSpin(this)">Putar Spin</button>
        <div id="spinResult" style="font-size:11px;color:var(--muted);text-align:center;margin-top:8px;font-weight:600"></div>
      </div>

      <div class="card">
        <div class="section-title">Toko Poin</div>
        <div class="point-shop">
          <div class="point-item"><div style="font-weight:800;font-size:12px">+1 Kuota</div><div style="font-size:10px;color:var(--muted)">1 kuota</div><div class="point-price">100 PTS</div><button class="btn btn-ghost" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('quota1', this)">Tukar</button></div>
          <div class="point-item"><div style="font-weight:800;font-size:12px">+3 Kuota</div><div style="font-size:10px;color:var(--muted)">Hemat</div><div class="point-price">250 PTS</div><button class="btn btn-ghost" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('quota3', this)">Tukar</button></div>
          <div class="point-item"><div style="font-weight:800;font-size:12px">Reset Spin</div><div style="font-size:10px;color:var(--muted)">Spin lagi</div><div class="point-price">150 PTS</div><button class="btn btn-ghost" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('spin', this)">Tukar</button></div>
          <div class="point-item"><div style="font-weight:800;font-size:12px">VIP 1H</div><div style="font-size:10px;color:var(--muted)">Unlimited</div><div class="point-price">500 PTS</div><button class="btn btn-dark" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('vip1', this)">Tukar</button></div>
          <div class="point-item"><div style="font-weight:800;font-size:12px">VIP 3H</div><div style="font-size:10px;color:var(--muted)">Best value</div><div class="point-price">1200 PTS</div><button class="btn btn-dark" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('vip3', this)">Tukar</button></div>
          <div class="point-item"><div style="font-weight:800;font-size:12px">+200 Bonus</div><div style="font-size:10px;color:var(--muted)">300->500</div><div class="point-price">300 PTS</div><button class="btn btn-dark" style="padding:8px;font-size:10px;margin-top:6px" onclick="redeemPoints('bonus200', this)">Tukar</button></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="viewOwnerArea" style="display:none">
    <div id="viewOwnerDashboard" class="view active">
      <div class="card">
        ...
        <div class="owner-stat-grid">
          <div class="stat"><div class="stat-v" id="oRev">Rp 0</div><div class="stat-l">Revenue</div></div>
          <div class="stat"><div class="stat-v" id="oUsers">0</div><div class="stat-l">Users</div></div>
          <div class="stat"><div class="stat-v" id="oPending">0</div><div class="stat-l">Pending</div></div>
          <div class="stat"><div class="stat-v" id="oPremium">0</div><div class="stat-l">VIP</div></div>
    ...
    <div id="viewOwnerUsers" class="view">
      <div class="card"><div class="section-title">Users</div><div id="oUserList"></div></div>
    </div>
    ...
    <div id="viewOwnerVoucher" class="view">
      <div class="card"><div class="section-title">Buat Voucher</div>
        ...
        <button class="btn btn-dark" onclick="createVoucher(this)">Buat</button>
    ...
    <div id="viewOwnerBroadcast" class="view">
      <div class="card"><div class="section-title">Broadcast</div>
        <textarea class="input" id="bcTextInput" style="min-height:120px;resize:none" placeholder="Pesan broadcast"></textarea>
        <button class="btn btn-dark" style="margin-top:8px" onclick="sendBroadcast(this)">Kirim Broadcast</button>
...
var wheelRotation=0;
var wheelPrizes=[
{label:"+50 PTS",color:"#0f172a"},
{label:"ZONK",color:"#e2e8f0"},
{label:"+25 PTS",color:"#334155"},
{label:"+100 PTS",color:"#475569"},
{label:"+3 KUOTA",color:"#64748b"},
{label:"VIP 1H",color:"#0f172a"}
];
var weeklyPoints=[
...
function hideLoader(){var ldr=document.getElementById('loader');if(!ldr) return;if(ldr.classList.contains('hidden')) return;ldr.classList.add('hidden');setTimeout(function(){ldr.style.display='none';},400);}
...
if(prog){prog.style.animation='none';void prog.offsetWidth;prog.style.animation='prog 3s linear forwards';}
...
ctx.fillText(wheelPrizes[i].label, r-14, 4);
ctx.restore();
}
ctx.beginPath();
...
if(tg){try{tg.ready();tg.expand();if(tg.setHeaderColor) tg.setHeaderColor('#ffffff');if(tg.setBackgroundColor) tg.setBackgroundColor('#fbfbfd');}catch(e){}}
...
if(currentUserId){try{localStorage.setItem('walzy_uid', String(currentUserId));}catch(e){}}
drawWheel();
setTimeout(hideLoader, 400);
setTimeout(hideLoader, 1200);
if(currentUserId){
loadUserData(false);
if(!pollingTimer){pollingTimer=setInterval(function(){loadUserData(true);}, 2500);}
if(!ownerPollingTimer){ownerPollingTimer=setInterval(function(){if(isUserOwner) loadOwnerData();}, 3500);}
}else{hideLoader();}
...
if(weekBadge) weekBadge.textContent='M'+(week+1);
var weeklyInfo=document.getElementById('weeklyInfo');
if(weeklyInfo){var pts=weeklyPoints[week];weeklyInfo.textContent='Minggu ini: D1 '+pts[0]+' | D2 '+pts[1]+' | D3 '+pts[2]+' | D4 '+pts[3]+' | D5 '+pts[4]+' | D6 '+pts[5]+' | D7 '+pts[6]+' PTS';}
...
var chkBtn=document.getElementById('checkinBtn');if(chkBtn){chkBtn.disabled=!u.canCheckin;chkBtn.textContent=u.canCheckin ? 'Check-in +'+bonusToday+' PTS' : 'Sudah Check-in';}
...
invBox.innerHTML='<div class="invoice"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-weight:800;font-size:12px">'+(inv.id||inv.invoice)+' • '+inv.days+'H • Rp '+(inv.amount||0).toLocaleString('id-ID')+'</div><span class="badge dark">'+stText+'</span></div>'+proofHtml+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px"><button class="btn btn-dark" style="padding:9px;font-size:11px" onclick="triggerUploadProof()">Upload Bukti</button><button class="btn btn-ghost" style="padding:9px;font-size:11px" onclick="cancelOrder(\\''+(inv.id||inv.invoice)+'\\', this)">Batal</button></div></div>';
...
return '<div class="invoice"><div style="display:flex;justify-content:space-between"><div><div style="font-weight:800;font-size:12px">'+p.id+' • '+p.days+'H • Rp '+(p.amount||0).toLocaleString('id-ID')+'</div><div style="font-size:10px;color:var(--muted)">User: '+p.userId+'</div>'+(p.proofImage ? '<div class="proof" style="margin-top:6px"><img src="'+p.proofImage+'" onclick="openZoomModal(\\''+p.proofImage+'\\')"></div>' : '<div style="font-size:10px;color:var(--amber);margin-top:4px">Belum bukti</div>')+'</div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px"><button class="btn btn-dark" style="padding:8px;font-size:10px" onclick="ownerAct(\\''+p.id+'\\', \\'approve\\', this)">Setujui</button><button class="btn btn-ghost" style="padding:8px;font-size:10px" onclick="ownerAct(\\''+p.id+'\\', \\'reject\\', this)">Tolak</button></div></div>';
}).join('');
...
return '<div class="card" style="padding:10px;margin-bottom:6px;box-shadow:none"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:800;font-size:12px">'+(u.first_name||'User')+'</div><div style="font-size:10px;color:var(--muted)">ID: '+u.id+' | Fix: '+(u.totalFix||0)+' | Poin: '+(u.points||0)+'</div></div><span class="badge '+(isVip?'dark':'')+'">'+(isVip ? 'VIP '+left : 'Free')+'</span></div></div>';
}).join('');
...
return '<div class="card" style="padding:10px;margin-bottom:6px;box-shadow:none"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:800;font-size:12px">'+c.code+' <span class="badge '+(status==='Aktif'?'dark':'')+'" style="font-size:8px;margin-left:4px">'+status+'</span></div><div style="font-size:10px;color:var(--muted);margin-top:2px">'+c.days+' Hari | '+quotaText+'</div></div><button class="btn btn-ghost" style="width:auto;padding:6px 10px;font-size:10px" onclick="deleteVoucher(\\''+c.code+'\\', this)">Hapus</button></div></div>';
}).join('');
