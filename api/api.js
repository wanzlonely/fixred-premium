...
module.exports = async (req, res) => {
  ...
  try{
    ...
    if(endpoint==='spin'){
      ...
      if(!dbUser){
        dbUser=ensureUserInDB(db,userId,firstName,null);
      }
    ...
    if(endpoint==='checkin'){
      ...
      if(!user){
        user=ensureUserInDB(db,userId,firstName,null);
      }
    ...
    if(endpoint==='stats'){
      ...
      return res.status(200).json({
        ...
        paidPayments:ownerCheck ? paid.slice(-50).reverse() : [],
        recentUsers:ownerCheck ? validUsers.slice(-50).reverse() : [],
        codes:ownerCheck ? Object.values(db.codes).slice(-100) : [],
    ...
    if(endpoint==='delete_code'){
      ...
      if(!db.codes[keyToDelete]){
        const found=Object.keys(db.codes).find(k=>k.toUpperCase()===code);
        if(found) keyToDelete=found;
      }
      if(!db.codes[keyToDelete]) return res.status(200).json({ok:false,message:'Voucher Tidak Ditemukan'});
    ...
    if(endpoint==='broadcast'){
      ...
      try{
        ...
        for(let u of validUsers){
          try{
            await bot.sendMessage(u.id, `📢 <b>WALZY STORE</b>\n\n${text}`, {parse_mode:'HTML'});
            sent++;
          }catch(e){ failed++; }
