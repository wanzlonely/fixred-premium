...
if(!checkRate(clientIp)) return res.status(429).json({ok:false,message:'Terlalu banyak request'});
...
if(!user) return res.json({ok:false,message:'User tidak ditemukan, /start dulu di bot'});
...
if(!userId) return res.status(400).json({ok:false,message:'user_id required'});
if(isSuspiciousId(userId)) return res.json({ok:false,message:'User tidak valid'});
if(!days || isNaN(days) || days<=0 || days>365) return res.json({ok:false,message:'Durasi 1-365'});
...
if(imageBase64.length>8*1024*1024) return res.json({ok:false,message:'Gambar max 5MB'});
...
return res.json({ok:true,message:'Bukti terkirim'});
}
...
if(code.length<3) return res.json({ok:false,message:'Kode min 3'});
if(isSuspiciousId(userId)) return res.json({ok:false,message:'User tidak valid'});
...
if(!code || code.length<3) return res.json({ok:false,message:'Kode min 3'});
...
if(text.length>1000) return res.json({ok:false,message:'Max 1000'});
const unique=getUniqueUsers(db.users);
res.json({ok:true,sent:unique.length,failed:0,message:'Broadcast mulai ke '+unique.length});
setImmediate(async()=>{
let sent=0,failed=0;
try{
const bot=new (require('node-telegram-bot-api'))(config.BOT_TOKEN);
...
if(i%20===0 && i>0){try{await bot.sendMessage(ownerId,`📢 Progress ${i}/${unique.length} ✅${sent} ❌${failed}`,{parse_mode:'HTML'});}catch(e){}}
}
try{
