...
async function loadDB() {
  try {
    ...
    if (!db) db = { users: {}, codes: {}, payments: {}, stats: { totalFix: 0, totalSuccess: 0, totalFailed: 0, revenue: 0 }, history: {}, pending: {}, supportMap: {} };
    ...
    return db;
  } catch (e) { return { users: {}, codes: {}, payments: {}, stats: { totalFix: 0, totalSuccess: 0, totalFailed: 0, revenue: 0 }, history: {}, pending: {}, supportMap: {} }; }
}
...
function rankLine(medal, name, refCount) {
  return `${medal} <b>${esc((name || 'User').substring(0, 20))}</b> ━ ${refCount || 0} Ref`;
}

const isValidNumber = (n) => /^\+?[0-9]{8,15}$/.test(n);

module.exports = { redis, loadDB, saveDB, getTodayString, getWIB, normalizeNumber, getLastKey, genID, genInvoiceID, esc, getRank, extractEntries, UI, rankLine, isValidNumber };
