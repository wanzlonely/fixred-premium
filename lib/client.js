    ...
    for (let botName of targets) {
      try {
        ...
        while (Date.now() - start < 20000) {
          ...
          if (msgs && msgs.length) {
            for (let m of msgs) {
              if (m.message && (m.message.includes(String(number).slice(-4)) || m.message.toUpperCase().includes('CPHX') || m.message.toLowerCase().includes('terkirim') || m.message.toLowerCase().includes('berhasil'))) {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      try {
        ...
        if (msgs && msgs.length) {
          for (let m of msgs) {
            const txt = m.message || '';
            if (txt.includes(numTail) || (targetIdStr && txt.includes(targetIdStr))) {
