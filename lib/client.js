...
async function getClient() {
  ...
  connecting = (async () => {
    try {
      ...
      client = new TelegramClient(session, Number(config.API_ID), config.API_HASH, { 
        connectionRetries: 3,
        timeout: 5000
      });
    ...
    for (let botName of targets) {
      try {
        ...
        while (Date.now() - start < 4000) {
          const msgs = await cli.getMessages(botName, { limit: 3 });
          if (msgs && msgs.length) {
            for (let m of msgs) {
              const txt = m.message || '';
...
async function monitorTargetResponse(number, targetId, timeoutMs = 8000) {
  try {
    const cli = await getClient();
